...
`analysis/model_visualization_comprehensive.ipynb` from the per-epoch training logs.
...
Summary figures: `assets/best_accuracy_comparison.png`,
`assets/training_curves_combined.png`, `assets/model_radar_comparison.png`.
...
├── analysis/                                hair removal + comparison notebooks
├── assets/                                  attention maps + comparison figures
...
dataset `ahmedxc4/skin-ds` and is passed through `analysis/hair_removal.ipynb`, which
...
Run `analysis/hair_removal.ipynb` once to produce `skin-ds-processed`, then open any model
notebook to train from scratch or evaluate from a checkpoint.
`analysis/model_visualization_comprehensive.ipynb` reads the `*_training_logs.csv` files
...
- Only `analysis/hair_removal.ipynb` fixes a seed (42). The other notebooks do not, so
